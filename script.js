document.addEventListener('DOMContentLoaded', function() {
	...
	function startGame() {
		...
		document.querySelector('#game button[onclick="endGame()"]').classList.remove('hidden');
		document.querySelector('#game button[onclick="resetScores()"]').classList.remove('hidden');
		renderPlayers();
	}
	...
	window.endGame = function() {
		...
		document.querySelector('#game button[onclick="endGame()"]').classList.add('hidden');
		document.querySelector('#game button[onclick="resetScores()"]').classList.add('hidden');
		displaySummary();
	};
/*
	window.resetScores = function() {
		// Use confirm dialog to ask if the user really wants to reset the scores
...
*/
	...
	window.actuallyResetScores = function() {
		players = players.map(player => ({ ...player, score: 0 }));
		localStorage.setItem('carCountGame', JSON.stringify(players));
		renderPlayers();
		closeModal();	
